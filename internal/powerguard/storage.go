...
// bayRootPorts 是实测的 M.2 仓位走线（2026-09 多轮丝印实测，插拔网卡、
// 换盘、空仓各状态复核）：仓位 N 挂根端口 1d.(N-1)，与整机 addon 的 dts
// 定义一致；Mellanox 在位只会挪 SATA 控制器的根端口（1c.0/1c.2），不影
// 响 M.2 绑定。空仓的根端口不被固件枚举，无法靠扫描建表，只能固化实测值。
